 ...
 * UCID: lap5
 * Date: 2025-11-23
 * Summary: Singleton Server class that listens for client connections, manages rooms, and handles server shutdown.
 */
package Server;
...
public enum Server {
    ...
    protected void joinRoom(String name, ServerThread client) throws RoomNotFoundException {
        ...
        if (currentRoom != null && currentRoom.getName().equalsIgnoreCase(nameCheck)) {
            info(String.format("Client %s tried to join the room they are already in (%s)", client.getDisplayName(),
                    name));
            return;
        }
        if (currentRoom != null) {
