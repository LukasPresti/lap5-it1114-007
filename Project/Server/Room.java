...
public class Room implements AutoCloseable {
    ...
    protected long hostId = Constants.DEFAULT_CLIENT_ID;
    ...
    protected synchronized void addClient(ServerThread client) {
        ...
        if (hostId == Constants.DEFAULT_CLIENT_ID) {
            ...
            // info("Assigned host to " + client.getDisplayName());
        }
    ...
    protected synchronized void removeClient(ServerThread client) {
        ...
        if (removedClient != null) {
            ...
            if (client.getClientId() == hostId) {
                // info("Host left, reassigning...");
                hostId = Constants.DEFAULT_CLIENT_ID;
                if (!clientsInRoom.isEmpty()) {
                    // Assign new host to the first available client
                    // In a real app, you might want more complex logic (e.g. oldest member)
                    hostId = clientsInRoom.keySet().stream().findFirst().orElse(Constants.DEFAULT_CLIENT_ID);
                }
                if (hostId != Constants.DEFAULT_CLIENT_ID) {
                    sendHostStatus(hostId);
                }
    ...
    protected void sendHostStatus(long hostId) {
        clientsInRoom.values().forEach(client -> {
            client.sendClientInfo(hostId, null, null, RoomAction.HOST);
        });
    }
        ...
        clientsInRoom.values().forEach(serverThread -> {
            if (serverThread.getClientId() != incomingClient.getClientId()) {
                boolean failedToSync = !incomingClient.sendClientInfo(serverThread.getClientId(),
                        serverThread.getClientName(), getName(), RoomAction.JOIN, true, serverThread.isAway());
                if (failedToSync) {
    ...
    private void joinStatusRelay(ServerThread client, boolean didJoin) {
        clientsInRoom.values().removeIf(serverThread -> {
            String suffix = " the room";
            if (didJoin && this instanceof GameRoom) {
                if (((GameRoom) this).getCurrentPhase() == Project.Common.Phase.IN_PROGRESS) {
                    suffix = " as a spectator";
                }
            }
