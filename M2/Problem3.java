...
// 3. Convert the value to positive using Math.abs (or parsing for strings)
// 4. Store it back into output[] in the same data type
...
for (int i = 0; i < arr.length; i++) {
    ...
    if (val instanceof Integer) {
        ...
        // try to detect if it's int or double
        String s = (String) val;
        if (s.contains(".")) {
            output[i] = Math.abs(Double.parseDouble(s));
        } else {
            output[i] = Math.abs(Integer.parseInt(s));
        }
    } else {
        // fallback for mixed objects (like in array5)
